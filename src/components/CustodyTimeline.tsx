...
export default function CustodyTimeline({ records, recalled }: { records: CustodyRecord[]; recalled?: boolean }) {
  if (records.length === 0) {
    return <p className="text-xs text-muted-foreground py-4">No custody records on Sui yet.</p>;
  }
        ...
        {records.map((record, idx) => (
          <div key={record.objectId} className="flex gap-3">
            <div className="flex flex-col items-center pt-1">
              ...
              {idx < records.length - 1 && <div className="w-px bg-border flex-1 min-h-[1.75rem] mt-1" />}
            </div>
            <div className="pb-5 min-w-0 flex-1">
              ...
              <div className="flex items-center gap-3 mt-0.5">
                <span className="text-xs text-muted-foreground">{record.quantity.toLocaleString()} units</span>
                <span className="text-xs text-muted-foreground">
                  {record.timestamp ? formatDistanceToNow(new Date(record.timestamp), { addSuffix: true }) : '—'}
                </span>
                <span className="text-xs font-mono text-muted-foreground/60">#{record.sequence}</span>
              </div>
            </div>
          </div>
        ))}
