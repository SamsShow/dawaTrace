...
interface MutationResult { success: boolean; message: string }

const CHEMIST_VIOLATIONS_QUERY = gql`
  query ChemistViolations {
    chemistViolations { chemistId violationCount suspended violationBatchIds lastViolationAt }
  }
`;
...
const LIFT_SUSPENSION_MUTATION = gql`
  mutation LiftSuspension($chemistId: String!) { liftSuspension(chemistId: $chemistId) { success message } }
`;

const BULK_RECALL_MUTATION = gql`
  mutation BulkRecall($batchIds: [String!]!, $reason: String!) { bulkRecall(batchIds: $batchIds, reason: $reason) { success message } }
`;

export function useChemistViolations() {
  return useQuery<{ chemistViolations: ChemistViolation[] }>(CHEMIST_VIOLATIONS_QUERY, { pollInterval: 30_000 });
}
...
export function useAdminOverview() {
  ...
  }>(ADMIN_OVERVIEW_QUERY, { pollInterval: 30_000 });
}
