...
export default function Dashboard() {
  ...
  const STATS = [
    ...
    { label: 'Recalled batches', value: recalls.length > 0 ? String(recalls.length) : '0' },
  ...
  return (
    <>
      <div className="px-6 py-4 border-b border-border"><h1 className="text-sm font-semibold">Overview</h1></div>
      ...
      <div className="px-6 py-6 space-y-6">
        {recalls.length > 0 && (
          <section>
            <h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-2">Active Recalls</h2>
            <div className="border border-destructive/40 divide-y divide-destructive/20">
              {recalls.map((r) => (
                <div key={r.batchId} className="px-4 py-3 flex items-start justify-between gap-4">
                  ...
                  <p className="text-[11px] text-muted-foreground shrink-0">{format(new Date(r.timestamp), 'd MMM, HH:mm')}</p>
                </div>
              ))}
            </div>
          </section>
        ...
        <section>
          <div className="flex items-center justify-between mb-3">
            <h2 className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Recent Batches</h2>
            <Link href="/batches" className="text-xs text-muted-foreground hover:text-foreground transition-colors flex items-center gap-1">View all <ArrowRight className="h-3 w-3" /></Link>
          </div>
          {batches.length === 0 ? (
            <div className="border border-border px-4 py-8 text-center">
              ...
              <Link href="/batches" className="text-xs underline mt-2 inline-block">Mint the first batch</Link>
            </div>
          ) : (
            <div className="border border-border divide-y divide-border">
              {[...batches].sort((a, b) => b.updatedAt - a.updatedAt).slice(0, 8).map((b) => (
                <Link key={b.batchId} href={`/batches/${b.batchId}`} className="flex items-center justify-between gap-4 px-4 py-3 hover:bg-muted/20 transition-colors">
                  ...
                  <div className="flex items-center gap-3 shrink-0">
                    {recalledBatchIds.has(b.batchId) ? <Badge variant="destructive">RECALLED</Badge> : <Badge variant={STATUS_VARIANT[b.status]}>{b.status.replace('_', ' ')}</Badge>}
                    <span className="text-[11px] text-muted-foreground">{format(new Date(b.updatedAt), 'd MMM')}</span>
                  </div>
                </Link>
              ))}
            </div>
