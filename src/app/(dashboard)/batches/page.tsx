    ...
    <>
      <div className="px-6 py-4 border-b border-border flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          {isRegulator && (
            <>
              <Button size="sm" variant="outline" onClick={() => downloadExport('/api/exports/batches?format=csv')}><Download className="h-3.5 w-3.5 mr-1.5" />Export CSV</Button>
              <Button size="sm" variant="outline" onClick={() => downloadExport('/api/exports/batches?format=json')}><Download className="h-3.5 w-3.5 mr-1.5" />Export JSON</Button>
            </>
          )}
          <Button size="sm" onClick={() => { setOpen(true); setMintSuccess(''); setMintError(''); }}><PackagePlus className="h-3.5 w-3.5 mr-1.5" />Mint batch</Button>
        </div>
      </div>
      ...
      <div className="px-6 py-6">
        {loading && batches.length === 0 ? (
          ...
          <div className="border border-border px-4 py-10 text-center">
            ...
            <Button size="sm" variant="outline" className="mt-3" onClick={() => setOpen(true)}>Mint the first batch</Button>
          </div>
        ) : (
          <div className="border border-border divide-y divide-border">
            ...
            {batches.map((b) => (
              <div key={b.batchId} className="grid grid-cols-[1fr_1fr_1fr_80px_100px_32px] gap-4 px-4 py-3 items-center hover:bg-muted/20 transition-colors">
                <div className="min-w-0">
                  <p className="text-xs font-mono truncate">{b.batchId}</p>
                  <p className="text-[11px] text-muted-foreground mt-0.5">{format(new Date(b.createdAt), 'd MMM yyyy')}</p>
                </div>
                <div className="min-w-0">
                ...
                <Link href={`/batches/${b.batchId}`} className="flex items-center justify-center text-muted-foreground hover:text-foreground transition-colors">
          ...
          <div className="bg-background border border-border w-full max-w-md shadow-lg flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleMint} className="px-5 py-4 space-y-3.5 overflow-y-auto">
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1.5"><Label htmlFor="batchId">Batch ID</Label><Input id="batchId" placeholder="BATCH-2026-004" value={form.batchId} onChange={set('batchId')} /></div>
                <div className="space-y-1.5"><Label htmlFor="quantity">Quantity (units)</Label><Input id="quantity" type="number" min="1" placeholder="10000" value={form.quantity} onChange={set('quantity')} /></div>
              </div>
              <div className="space-y-1.5"><Label htmlFor="drugName">Drug name</Label><Input id="drugName" placeholder="Amoxicillin 500mg" value={form.drugName} onChange={set('drugName')} /></div>
              <div className="space-y-1.5"><Label htmlFor="composition">Composition</Label><Input id="composition" placeholder="Amoxicillin trihydrate 574mg eq. to..." value={form.composition} onChange={set('composition')} /></div>
              <div className="grid grid-cols-2 gap-3">
                ...
                <div className="space-y-1.5"><Label htmlFor="licenseNo">Mfg. license no.</Label><Input id="licenseNo" placeholder="MFG-DL-2024-1182" value={form.licenseNo} onChange={set('licenseNo')} /></div>
              </div>
